...
class LichessObserver{
    ...
    async replaceGame(replaceId, excludeIds){
        ...
        const response = await fetch(`https://lichess.org/games/${this.gameType}/replacement/` + replaceId + '?' + exclude.toString())
    ...
    initWs(){
        ...
        this.ws = new WebSocket(`wss://socket2.lichess.org/socket/v5?sri=${Math.random().toString(36).slice(2, 12)}`)
