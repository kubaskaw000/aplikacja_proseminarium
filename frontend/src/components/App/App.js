...
const getGamesInfo = async (fen) => {
  ...
  try {
    const response = await fetch(`http://localhost:5000/api/lichess-games?fen=${fen}`)
    ...
    <><div className="app">
      <div className="app__board">
        ...
        <div className="board__input">
          ...
          <span>FEN</span>
          <input className="board__input__fen" type="text" value={inputFen} onChange={e => setInputFen(e.target.value)} onKeyDown={handleGamesUpdate}></input>
      ...
      <div className="chess_analyser">
        <select className="game__selector" onChange={e => setGames({ ...games, index: e.target.value })}>
        ...
        {console.log(game)}
        <Analyser moves={game.moves || []} currentMoveIndex={game.currentMoveIndex} updateGameMove={updateGameMove} />
      </div>
    </div>
    </>
  );
