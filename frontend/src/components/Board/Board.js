    ...
    for (let i = 0; i < coordsX.length; i++) {
      if (currentBoard[j][i] !== null) {
        pieces.push({
          ...
          imgSrc: `/img/${
            currentBoard[j][i].color + currentBoard[j][i].type
          }.svg`,
        });
      } else {
        pieces.push({
        ...
        <>
          <div
            ...
            className={fieldClass(
              ...
              id === lastMove?.to,
              inCheck &&
                piecesInfo[fieldNr]?.type === "k" &&
                piecesInfo[fieldNr]?.color === chess.turn()
            )}
          >
            {piecesInfo[fieldNr].imgSrc && (
              <img
                alt="Loading"
                className="piece"
                src={piecesInfo[fieldNr].imgSrc}
              />
            )}
          </div>
        </>
